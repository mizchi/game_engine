...
- `moon test --target native`: 133 passed / 0 failed
- `moon test --target js`: 128 passed / 0 failed
...
| CommandQueue 集約/flush | `internal/graphicscommand/commandqueue.go` | `SimpleCommandQueue` で pipeline/texture(src_image_ids)/blend/uniform/index 条件の merge を実装（explicit geometry payload は非merge） | 部分 |
